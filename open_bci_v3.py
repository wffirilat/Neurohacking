...
class OpenBCIBoard(object):
    ...
    def __init__(self, port=None, baud=115200, filter_data=True,
                 scaled_output=True, daisy=False, aux=False, impedance=False, log=True, timeout=1):
        #This should now time out after 5 seconds
        ...
        for rep in range(max_bytes_to_skip):
            ...
            if self.read_state == 0:
                ...
                if struct.unpack('B', b)[0] == START_BYTE:
                    if rep != 0:
                        self.warn('%d bytes before start found and skipped' % rep)
            ...
            # ---------Channel Data---------
                ...
                for c in range(self.eeg_channels_per_sample):
                    ...
                    val = struct.unpack('>i', literal_read)[0]

                    if self.scaling_output:
                        channel_data.append(val * scale_fac_uVolts_per_count)
                    else:
    ...
    def openbci_id(ser):
        ...
        if ser.inWaiting():
            ...
            # Look for end sequence $$$
            while '$$$' not in line:
                c = ser.read().decode('utf-8', errors='replace')  # we're supposed to get UTF8 text, but the board might behave otherwise
                line += c
            if "OpenBCI" in line:
                return True
        return False
    ...
    def set_channel(self, channel, toggle_position):
        ...
        if not 1 <= channel <= 16:
            self.warn("Channel must be between 1 and 16.")
        if channel > 8 and not self.daisy:
            self.warn("Channel can only be > 8 in daisy mode.")

        if toggle_position:
            channel_ids = b'!@#$%^&*QWERTYUI'
        else:
            channel_ids = b'12345678qwertyui'

        self.ser.write(channel_ids[channel])
    ...
    def find_port(self):
        ...
        if sys.platform.startswith('win'):
            ...
            raise EnvironmentError('Don\'t know how to find ports on this operating system')
        openbci_port = ''
        for port in ports:
            try:
